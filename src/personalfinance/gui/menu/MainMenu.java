...
public class MainMenu extends JMenuBar implements Refresh, EnableEditDelete {
    ...
    private JMenuItem addMenuItem(JMenu jMenu, String title, ImageIcon icon, String action, int key){
        JMenuItem jMenuItem = new JMenuItem();
        jMenuItem.setIcon(icon);
        jMenuItem.setText(title);
        jMenuItem.setActionCommand(action);
        if (key != 0){
            KeyStroke keyStroke = KeyStroke.getKeyStroke(key, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask());
    ...
    private JMenuItem addMenuItem(JMenu jMenu, String title, ImageIcon icon, String action){
        return addMenuItem(jMenu, title, icon, action, 0);
    }
